...
const urlCreation = async function (req, res) {
  try {
    ...
    if (!longUrl)
      return res.status(400).json({ message: "Please enter longUrl" });

    if (typeof longUrl !== "string")
      return res
        .status(400)
        .json({ message: "Url should be in string format" })

    //==== Regex use Valid Url =====

    let reg = /^(https:\/\/www\.|http:\/\/www\.|www\.|https:\/\/|http:\/\/)[^www.,-_][a-zA-Z0-9\-_.$]+\.[a-zA-Z]{2,5}(:[0-9]{1,5})?(\/[^\s]*)$/gm;

// const reg = /^(https?:\/\/)?(www\.)?[a-z0-9]+\.[a-z]+(\/[^\s]*)?$/i;
...
const getUrl = async function (req, res) {
  try {
    ...
    return res
  .status(400)
  .json({ message: "please Enter urlCode only in lowercase" });
    ...
    if (!urlCodeExist)
      return res
        .status(404)
        .json( { message: "urlCode not found" } );
